...
def dc( ax, theta, r=1., ox=0., oz=0., color='k' ) :
  ...
  rads = np.arange( 0., np.pi/2., 0.01, dtype=float ) -np.pi/4.+ theta
  ...
  rads = np.arange( 0., np.pi/2., 0.01, dtype=float ) + np.pi*3./4.+ theta
  ...
  rads = np.arange( -np.pi, np.pi, 0.01, dtype=float )
